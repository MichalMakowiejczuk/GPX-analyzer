...
class TrackDataProcessor:
    ...
    def __post_init__(self) -> None:
        ...
        self.track_df = self._smooth_elevation_profile(
            df, self.smooth_window, self.min_distance_m
        )
        self.track_df = self._assign_segments_by_distance(
            self.track_df, self.seg_unit_km
        )

    @staticmethod
    def _smooth_elevation_profile(
        df: pd.DataFrame, mean_window: int = 5, min_distance_m: float = 1.0
    ) -> pd.DataFrame:
    ...
    @staticmethod
    def _assign_segments_by_distance(
        df: pd.DataFrame, seg_unit_km: float
    ) -> pd.DataFrame:
        out = df.copy()
